...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $get_money_cash = $uf_data ? $this->num_uf($input['calculatorDisplay']) : $input['calculatorDisplay'];
        ...
        $transaction = Transaction::create([
            ...
            'cash_register_detail_id' => $input['cash_register_detail_id'],
            'type' => $sale_type,
            'status' => $input['status'],
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $output['tse_finish'] = "";
        $output['tse_trx_nr'] = "";
        $output['tse_signatur_nr'] = "";
        $output['tse_signatur'] = "";
        $output['tse_serial_nr'] = "";
        $output['fcc_transaction_response'] = $transaction->fcc_transaction_response;

        if($transaction->fcc_transaction_response){
           $tse_start_respons =  json_decode($transaction->fcc_transaction_response);
           $output['tse_start'] = $tse_start_respons->logTime;
           $output['tse_trx_nr'] = $tse_start_respons->transactionNumber;
           $output['tse_serial_nr'] = $tse_start_respons->serialNumber;
        }

        if($transaction->fcc_finish_transaction_response){
            $tse_finish_respons = json_decode($transaction->fcc_finish_transaction_response);
            $output['tse_finish'] = $tse_finish_respons->logTime;
            $output['tse_signatur_nr'] = $tse_finish_respons->signatureCounter;
            $output['tse_signatur'] = $tse_finish_respons->signatureValue;
        }
        $output['tse_qr_code'] = "V0;".$eas->seriennumber.";Kassenbeleg-V1;Beleg^".$summe_tax_19."_".$summe_tax_7."_".$summe_tax_10_7."_".$summe_tax_5_5."_".$summe_tax_0."^".$transaction->final_total.":".$payment_type."
        ;".$tse_start_respons->transactionNumber.";".$tse_finish_respons->signatureCounter."
        ;".$tse_start_respons->logTime.";".$tse_finish_respons->logTime.";ecdsa-plain-
        SHA384;utcTime;".$tse_finish_respons->signatureValue.";". base64_encode($tse_finish_respons->serialNumber);
        ...
        if ($transaction_type == 'sell' && $transaction->status == 'final') {
            ...
            $get_money_cash = $transaction->get_money_cash;
            $return_money = $transaction->get_money_cash - $paid_amount;
            $output['total_get_money_cash'] = ($get_money_cash == 0) ? 0 : $this->num_f($get_money_cash, $show_currency, $business_details);
            $output['total_return_money'] = ($get_money_cash == 0) ? 0 : $this->num_f($return_money, $show_currency, $business_details);
        ...
        if (in_array($transaction_type, ['sell', 'sales_order'])) {
            ...
            if ($transaction->status == 'final') {
                ...
                $output['qr_code_text'] = $this->getInvoiceUrl($transaction->id, $business_details->id);
            }
        }
    ...
    public function createExpense($request, $business_id, $user_id, $format_data = true)
    {
        $transaction_data = $request->only(['ref_no', 'transaction_date',
            ...
            'expense_category_id', 'cash_register_detail_id'/*'tax_id', 'contact_id', */ ]);

        $transaction_data['business_id'] = $business_id;
        $transaction_data['cash_register_detail_id'] = $request->input('cash_register_detail_id');
    ...
    public function getSellNetTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null, $permitted_locations = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
                    ->where('transactions.type', 'sell')
                    ->where('transactions.status', 'final')
                    ->leftjoin('transaction_sell_lines as S', 'transactions.id', '=', 'S.transaction_id')
                    ->select(
                        DB::raw('SUM(S.unit_price) as total_sell_netto')
                    );
        ...
        $output['total_sell_netto'] = $sell_details->total_sell_netto;
