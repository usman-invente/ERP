...
class CashRegisterUtil extends Util
{
    ...
    public function countOpenedCashRegister($business_id, $location_id, $cash_register_detail_id)
    {
        // $user_id = auth()->user()->id;
        // $count = CashRegister::where('user_id', $user_id)
        //                         ->where('status', 'open')
        ...
        $count = CashRegister::where('business_id',$business_id)
    ...
    {
        $user_id = auth()->user()->id;
        $register = CashRegister::where('cash_register_detail_id', $transaction->cash_register_detail_id)
     ...
     * @return bool
     */
    public function addExpensePayments($transaction)
    {
        $user_id = auth()->user()->id;
        $register = CashRegister::where('cash_register_detail_id', $transaction->cash_register_detail_id)
                                // ->where('user_id', $user_id)
    ...
    public function getRegisterDetails($register_id = null)
    {
        ...
        $register_details = $query->select(
            ...
            'cash_registers.cash_register_detail_id',
            'cash_registers.id',
     ...
     * @return array
     */
    public function getRegisterTransactionDetails($user_id, $open_time, $close_time, $is_types_of_service_enabled = false, $location_id = null, $cash_register_detail_id = null)
    {
        $product_details_by_brand = Transaction::where('transactions.location_id', $location_id)
        ...
        $product_details = Transaction::where('transactions.location_id', $location_id)
                // ->where('transactions.created_by', $user_id)
                ->where('transactions.cash_register_detail_id', $cash_register_detail_id)
                ->whereBetween('transactions.created_at', [$open_time, $close_time])
                ->where('transactions.type', 'sell')
                ->where('transactions.status', 'final')
                ->where('transactions.is_direct_sale', 0)
        ...
        if ($is_types_of_service_enabled) {
            $types_of_service_details = Transaction::where('transactions.location_id', $location_id)
                // ->where('transactions.created_by', $user_id)
     ...
     * @return obj
     */
    public function getCurrentCashRegister($cash_register_detail_id)
    {
        $register = CashRegister::where('cash_register_detail_id', $cash_register_detail_id)
                                // ->where('user_id', $user_id)
                                ->where('status', 'open')
                                ->first();
	...
	public function getTransactionSellReturnSums($business_id, $location_id, $cash_register_detail_id, $open_time, $close_time )
    {
        $transactions_sell_return_sums = Transaction::where('transactions.business_id', $business_id)
        ...
        $sum = 0;
        foreach($transactions_sell_return_sums as $transactions_sell_return_sum)
        {
            $sum += $transactions_sell_return_sum->final_total;
        }
        return $sum;
    }

    public function IfCashRegisterOpenIs($business_id, $cash_register_detail_id){
        ...
        if(is_object($register) && $register->status == "open")
            return true;
        else
            return false;
    }
    ...
    public function getIcomesSumByCashRegisterId($cash_register_id, $cash_register_detail_id){

        $sum = Income::where("cash_register_detail_id", $cash_register_detail_id)
