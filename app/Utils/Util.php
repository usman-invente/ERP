...
class Util
{
     ...
     * 
    */
    public function func_diff_date($date1, $date2) {
        $now        = strtotime($date1);
    ...
    public function is_date_between_two_other_date($date1, $other_date1, $other_date2){
        if($date1 >= $other_date1 &&  $date1 <= $other_date2)
            return true;
        else 
            return false;
    }
    ...
    public function convertInputNumber($value)
    {
        $to_return = $value;
        if (preg_match('/^[\+-]?\d{1,3}(?:\.\d{3})*(?:,\d+)?$|^\d+(?:,)?\d+$/m', $value)){
            $tmp_value = str_replace(".", "", $value);
            $to_return = str_replace(",", ".", $tmp_value);
        }

        // if we've got ZERO value (0,00|0.00) we set it to null
        // otherwise some validations will fail if the field is not required, but min:0.01
        #if ($to_return == 0)
        #    $to_return = null;
    ...
    public function convertValueForView($value)
    {
        if(strlen($value) > env('MAX_DIGIT'))
            return $value;
