...
class BusinessUtil extends Util
{
    ...
    public function newBusinessDefaultResources($business_id, $user_id)
    {
        ...
        //Create Employee role for a new business
        $employee_role = Role::create(['name' => 'Employee#'.$business_id,
            'business_id' => $business_id,
            'guard_name' => 'web',
        ]);
        $employee_role->syncPermissions(['customer.view', 'customer.create', 'customer.update',
                                    'product.view','sell.view', 'sell.create', 'sell.update', 
                                    'edit_pos_payment', 'print_invoice', 'direct_sell.view',
                                    'view_paid_sells_only','view_due_sells_only','view_partial_sells_only',
                                    'view_overdue_sells_only', 'direct_sell.access', 'direct_sell.update',
                                    'view_commission_agent_sell', 'sell.payments', 'edit_sell_payment',
                                    'discount.access', 'access_types_of_service', 'access_sell_return', 'access_own_sell_return',
                                    'draft.view_all', 'draft.update', 'draft.delete',
                                    'quotation.view_all', 'quotation.update', 'quotation.delete',
                                    'access_shipping', 'access_pending_shipments_only', 'access_commission_agent_shipping',
                                    'view_cash_register', 'close_cash_register', 'brand.view', 'tax_rate.view',
                                    'unit.view', 'category.view', 'access_printers', 'dashboard.data',
                                    'access_default_selling_price', 'crm.access_contact_login', 'crm.access_sources',
                                    'crm.access_life_stage', 'crm.access_proposal', 'crm.access_all_schedule',
                                    'crm.access_all_leads', 'crm.access_all_campaigns'                                     
                                ]);
    ...
    public function createNewBusiness($business_details)
    {
        ...
        $business_details['enable_inline_tax'] = true;
    ...
    public function addLocation($business_id, $location_details, $invoice_scheme_id = null, $invoice_layout_id = null)
    {
        ...
        $location = BusinessLocation::create(['business_id' => $business_id,
            ...
            'house_nr' => $location_details['house_nr'],
            ...
            'email' => $location_details['business_email'],
    ...
    /**
     * Adds a new cash register to a business location
     *
     * @param  int  $business_id
     * @param  array  $location_details
     * @param  int  $invoice_layout_id default null
     * @return location object
     */
    public function addCashRegisterDetails($business_id, $business_location, $cash_register_details)
    {        
        $cash_register = new CashRegisterDetail();
        $cash_register->business_id = $business_id;
        $cash_register->location_id  = $business_location->id;
        $cash_register->name  = "First Cash Register";
        $cash_register->description  = $business_location->name. " - First Cash Register";
        $cash_register->cash_register_active = true;
        $cash_register->tss_active = true;
        $cash_register->save();
