...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            $menu->dropdown(
                    ...
                    function ($sub) {
                        $sub->url(
                            ...
                            __('income.list_incomes'),
                            ['icon' => '', 'active' => request()->segment(1) == 'incomes' && request()->segment(2) == null]
                        );
                        

                        // if (auth()->user()->can('income.add') || auth()->user()->can('expense.edit')) {
                            $sub->url(
                                action([\App\Http\Controllers\IncomeCategorieController::class, 'index']),
                                __('income.income_categories'),
                                ['icon' => '', 'active' => request()->segment(1) == 'income-categories']
                            );
                        // }
                    },
                    ['icon' => ' <svg aria-hidden="true" class="tw-size-5 tw-shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
            ...
            if (auth()->user()->can('purchase_n_sell_report.view') || auth()->user()->can('contacts_report.view')
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        //   if(session('business.id') == 1){
                            if(is_object($active_subscription) && in_array($active_subscription->package_id, $business->getAllowedPackages())){
                                $sub->url(
                                    action([\App\Http\Controllers\CashBookController::class, 'index']),
                                    __('cash_book.cash_book'),
                                    ['icon' => '', 'active' => in_array(request()->segment(2), ['cash-book'])]
                                );
                            }
                        // }
            ...
            {
            $menu->dropdown(
                __('lang_v1.modul_name_reminder'),
                function ($sub) {
                    $sub->url(
                        action([\App\Http\Controllers\ContractController::class, 'getContractExtensionThisWeek']),
                        __('contract.contract_extension'),
                        ['icon' => 'fa fas fa-contract', 'active' => request()->segment(1) == 'contracts'&& !(str_contains(request()->segment(2) , 'details'))]
                    );
                    $sub->url(
                        action([\App\Http\Controllers\ContactController::class, 'getAllContactsWithDobToday']),
                        __('contact.dobs'),
                        ['icon' => '', 'active' => request()->segment(1) == 'contacts' && (str_contains(request()->segment(2) , 'dob'))]
                    );
                
                },
                ['icon' => 'fas fa-file-contract fa-lg']
                )->order(7);
                // $menu->url(action([\App\Http\Controllers\ContractController::class, 'getContractExtensionThisWeek']), __('contract.contracts'), ['icon' => 'fas fa-file-contract', 'active' => request()->segment(1) == 'contracts'])->order(46);
            }
            //Kitchen menu
