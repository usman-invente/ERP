    ...
    */
    ...
    public function postRegister(Request $request)
    {
        ...
        try {
            $validator = $request->validate(
                ...
                [
                    ...
                    'street.required' => __('validation.required', ['attribute' => __('business.street')]),
                    'house_nr.required' => __('validation.required', ['attribute' => __('business.house_nr')]),
                    ...
                    'email.business_email' => __('validation.email', ['attribute' => __('business.email')]),
        ...
        'tax_label_2', 'tax_number_2', */]);
            ...
            $business_location = $request->only(['name', 'country', 'street', 'house_nr', 'state', 'city', 'zip_code', 
                /*'landmark',*/
    'website', 'mobile', 'business_email' /*'alternate_number',*/ ]);
            ...
            // }
            $business_details['ip_address'] = request()->ip();   
            ...
            $preSharedKey = env('SECONDARY_APP_TOKEN');
            $preSharedKey = env('PRE_SHARED_KEY');
            $tokenGenerationUrl = 'https://marketing.neoburg.net/api/api/auth/token';

            $tokenResponse = Http::withHeaders([
                'Authorization' => 'Bearer ' . $preSharedKey,
            ])->post($tokenGenerationUrl);
            $response = $tokenResponse->body();
            $decodedResponse = json_decode($response, true); 
            $response = Http::withToken($decodedResponse['token'])->post(env('SECONDARY_APP_URL'), [
                ...
                'street' => $business_location['street'],
            ...
            $cash_register_detail = $this->businessUtil->addCashRegisterDetails($business->id, $new_location, $business_location);
