...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $number_of_this_week = $contract->getNumberOfContracts($business_id, "this_week");
        $number_of_this_month = $contract->getNumberOfContracts($business_id, "this_month");
        $number_of_next_month = $contract->getNumberOfContracts($business_id, "next_month");
        $number_of_overdue = $contract->getNumberOfContracts($business_id, "overdue");

        $contact = new Contact();
        $number_dobs_today = count($contact->getBirthdayByCondition($business_id, 'today')) ;
        $number_dobs_this_week = count($contact->getBirthdayByCondition($business_id, 'this_week')) ;
        $number_dobs_this_month = count($contact->getBirthdayByCondition($business_id, 'this_month')) ;
        $number_dobs_next_month = count($contact->getBirthdayByCondition($business_id, 'next_month')) ;

        return view('home.index', compact('sells_chart_1', 'sells_chart_2', 'widgets', 'all_locations', 'common_settings',
                                         'is_admin','active_subscription','business',
                                         'number_of_this_week','number_of_this_month','number_of_next_month','number_of_overdue',
                                        'number_dobs_today', 'number_dobs_this_week', 'number_dobs_this_month', 'number_dobs_next_month'));
    }
    ...
    public function getTotals()
    {
        if (request()->ajax()) {
            ...
            $sell_details = $this->transactionUtil->getSellTotals($business_id, $start, $end, $location_id, null, $permitted_locations);
            ...
            $contracts_details = $this->contractUtil->getContractInfo($business_id, $start, $end, $location_id);
            ...
            $total_sell_netto = ! empty($sell_net_details['total_sell_netto']) ? $sell_net_details['total_sell_netto'] : 0;
            $output['total_sell_netto'] = $total_sell_netto;

            $total_contracts_fee_monthly = ! empty($contracts_details['total_contracts_fee_monthly']) ? $contracts_details['total_contracts_fee_monthly'] : 0;
            $total_contracts_discount = ! empty($contracts_details['total_contracts_discount']) ? $contracts_details['total_contracts_discount'] : 0;
            $total_contracts_price_total = ! empty($contracts_details['total_contracts_price_total']) ? $contracts_details['total_contracts_price_total'] : 0;
            $total_contracts_count = ! empty($contracts_details['total_contracts_count']) ? $contracts_details['total_contracts_count'] : 0;

            $output['total_contracts_fee_monthly'] = $total_contracts_fee_monthly;
            $output['total_contracts_discount'] = $total_contracts_discount;
            $output['total_contracts_price_total'] = $total_contracts_price_total;
            $output['total_contracts_count'] = $total_contracts_count;
