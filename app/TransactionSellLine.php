...
class TransactionSellLine extends Model
{
    ...
    public function so_line()
    {
     ...
     public function getTaxRateValue($transaction_id, $tax_id, $type){
        ...
        foreach ($transaction_sell_lines as $key => $transaction_sell_line) {
            if($type == "Netto"){
                $sum_value = $sum_value + ($transaction_sell_line->unit_price * $transaction_sell_line->quantity);
            }
            if($type == "Brutto"){
                $sum_value = $sum_value + ($transaction_sell_line->unit_price_inc_tax * $transaction_sell_line->quantity);
            }
            if($type == "Sum"){
                $sum_value = $sum_value + ($transaction_sell_line->item_tax * $transaction_sell_line->quantity);
            }
        }
    ...
    public function getSumAllTaxRates($transaction_id){
    ...
    public function getSumNettoValue($transaction_id){
