...
class Contact extends Authenticatable
{
    ...
    {
        $all_contacts = Contact::where('contacts.business_id', $business_id)
                        ->whereIn('contacts.type', ['customer', 'both', 'lead'])
                        ->active();
    ...
    public function getFullNameAndTitleById($id){
        $contact = Contact::where('id', $id)->first();

        $name_array = [];
        if (! empty($contact->title)) {
            $name_array[] = $contact->title;
        }
    ...
    public function getBirthdayByCondition($business_id, $condition){
        ...
        if($condition == "today"){
            ...
            foreach ($contacts as $key => $contact) {
                ...
                // if( date('m', strtotime($contact->dob)) === date('m', strtotime($today)) &&  date('d', strtotime($contact->dob)) === date('d', strtotime($today)))
                if ($today->format('m-d') == $dob->format('m-d')){
                    if (!in_array($contact, $array_today))
                        $array_today[] = $contact;
                }
            }
            // dd($nr_this_week);
            return $array_today;
        }elseif($condition == "this_week"){
            $FirstDayOfWeek = date("m-d", strtotime('monday this week'));  
            $LastDayOfWeek = date("m-d", strtotime('sunday this week'));

            $today = date("m-d"); 
            foreach ($contacts as $key => $contact) {
                $dob = date("m-d", strtotime( $contact->dob ));
                if($dob >= $today &&  $dob <= $LastDayOfWeek)
                    if (!in_array($contact, $array_this_week))
                        $array_this_week[] = $contact;
            }
            return $array_this_week;
        }
        elseif($condition == "this_month"){
            $FirstDayThisMonth = date('m-d', strtotime('first day of this month'));
            $LastDayThisMonth = date('m-d', strtotime('last day of this month'));
            $today = date("m-d");

            foreach ($contacts as $key => $contact) {
                $dob = date("m-d", strtotime( $contact->dob ));
                if($dob >= $today && $dob >= $FirstDayThisMonth &&  $dob <= $LastDayThisMonth)
                    if (!in_array($contact, $array_this_month))
                        $array_this_month[] = $contact;
            }
            return $array_this_month;
        }
        elseif($condition == "next_month"){
            $FirstDayNextMonth = date('m-d', strtotime('first day of next month'));
            $LastDayNextMonth = date('m-d', strtotime('last day of next month'));

            foreach ($contacts as $key => $contact) {
                $dob = date("m-d", strtotime( $contact->dob ));
                if($dob >= $FirstDayNextMonth &&  $dob <= $LastDayNextMonth)
                    if (!in_array($contact, $array_next_month))
                        $array_next_month[] = $contact;
            }
