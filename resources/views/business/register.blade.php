...
@section('content')
    ...
    <div class="col-md-8 col-xs-12 col-md-offset-2 tw-mt-6">
        <div
            ...
            {{-- @include('business.partials.register_zahlung_stripe') --}}
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        const stripe = Stripe("{{ env('STRIPE_TEST_PUB_KEY') }}");

        const checkoutButton = document.getElementById('stripe-checkout-button');
        checkoutButton.addEventListener('click', function () {
            fetch("{{ route('stripe.create.checkout.session', ['package_id' => $package_id]) }}", {
                method: "POST",
                headers: {
