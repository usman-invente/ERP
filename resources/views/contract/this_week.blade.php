...
@component('components.widget', ['class' => 'box-primary', 'title' => __('contract.contract_this_week')])
      
		    <table class="table table-bordered table-striped" id="contract_this_week_table" style="width: 80%">
                <thead>
                    <tr>
                 ...
                 <tbody></tbody>
                {{-- <tbody>
                    @foreach($class_contract->getContractsByCondition( $business->id, 'this_week') as $key => $contract)
                       @php
                            $date_to_contact=date_create($contract->date_to_contact);
                            $contract_start_date=date_create($contract->contract_start_date);
                            $contract_end_date=date_create($contract->contract_end_date);
                        @endphp
                        <tr>
                            <td> </td>
                            <td>{{ $contract->customer->name }}</td>
                            <td>{{ $contract->number }}</td>
                            <td>{{ $contract->connected_to_number }}</td>
                            <td>{{ $contract->contract_feld1 }}</td>
                            <td>{{ $contract->contract_feld2 }}</td>
                            <td>{{ $contract->contract_feld3 }}</td>
                            <td>{{ $contract->contract_feld4 }}</td>
                            <td>{{ $contract->contract_feld5 }}</td>
                            <td>{{ number_format($contract->fee_monthly, 2, ',', '.') . ' €'}}</td>
                            <td>{{ number_format($contract->discount, 2, ',', '.') . ' €'}}</td>
                            <td>{{ $contract->contract_feld6 }}</td>
                            <td>{{ $contract->discount_duraction }}</td>
                            <td>{{ number_format($contract->price_total, 2, ',', '.') . ' €'}}</td>
                            @if($contract->contract_start_date)
                                <td>{{ date_format($contract_start_date , ("d/m/Y")) }}</td>
                            @else
                                <td> </td>
                            @endif
                            @if($contract->contract_end_date)
                                <td>{{ date_format($contract_end_date , ("d/m/Y")) }}</td>
                            @else
                                <td> </td>
                            @endif
                            <td>{{ $contract->contract_duraction }}</td>
                            <td>{{ $contract->contact_before_end_of_contract }}</td>
                            @if($contract->date_to_contact)
                                <td>{{ date_format($date_to_contact , ("d/m/Y")) }}</td>
                            @else
                                <td> </td>
                            @endif
                        </tr>
...
@section('javascript')
    <script type="text/javascript">
        ...
        $(document).ready(function() {
                    ...
                    $('#contract_this_week_table thead tr').clone(true).appendTo( '#contract_this_week_table thead' );
					$('#contract_this_week_table thead tr:eq(0) th').each( function (i) {
						var title = $(this).text();
        			    if(i == 0 ){
        			       $(this).html( '<td/>' ); 
        			    }
                    ...
                    contract_modul_datatable = $("#contract_this_week_table").DataTable({
	        			processing: true,
	        	        serverSide: true,
