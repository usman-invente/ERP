...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	<td>
		...
		@php
			$product_name = ' ';
			if(!empty($product->brand)){$product_name = '<br/> Marke: ' . $product->brand ;}
			$product_name .= '<br/> Produkt: '.$product->product_name . '<br/> P-ID: ' . $product->sub_sku ;
			// if(!empty($product->brand)){$product_name .= ' ' . $product->brand ;}
		@endphp
...
<br>
	...
	<td class="text-center">
		...
		@endphp
		<input readonly type="{{$subtotal_type}}" class="form-control pos_line_total @if(!empty($pos_settings['is_pos_subtotal_editable'])) input_number @endif" value="{{@num_format($product->quantity_ordered*$unit_price_inc_tax )}}">
