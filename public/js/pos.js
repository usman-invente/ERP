...
$(document).on('click', '#add_income', function(){
    $.ajax({
        ...
        dataType: 'html',
        success: function(result) {
            $('#income_modal').html(result);
            $('#income_modal').modal('show');
        },
    });
});

$(document).on('shown.bs.modal', '#income_modal', function(){
    $('#income_transaction_date').datetimepicker({
        format: moment_date_format + ' ' + moment_time_format,
        ignoreReadonly: true,
    });
    $('#income_modal .paid_on').datetimepicker({
        format: moment_date_format + ' ' + moment_time_format,
        ignoreReadonly: true,
    });
    $(this).find('.select2').select2();
    $('#add_income_modal_form').validate();
});

$(document).on('hidden.bs.modal', '#income_modal', function(){
    $(this).html('');
});

$(document).on('submit', 'form#add_income_modal_form', function(e) {
    e.preventDefault();
    var data = $(this).serialize();
