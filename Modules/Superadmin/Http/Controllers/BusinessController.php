...
class BusinessController extends BaseController
{
    ...
    public function edit_subscription($id, $sub_id)
    {
        $subscription = Subscription::findOrFail($sub_id);
        return view('subscriptions.edit', compact('subscription'));
    }

    public function update_subscription(Request $request, $id, $sub_id)
    {
        $subscription = Subscription::findOrFail($sub_id);

        $business = Business::with(['currency', 'locations', 'subscriptions', 'owner'])->find($id);
        ...
        $subscription->update($request->only(['package_name', 'start_date', 'trial_end_date', 'end_date']));

        return view('superadmin::business.show')
            ->with(compact('business', 'created_by'));
        //return redirect()->route('subscriptions.index')->with('success', 'Subscription aktualisiert.');
    }

    public function destroy_subscription($id, $sub_id)
    {
        $subscription = Subscription::findOrFail($sub_id);
        $subscription->delete();
        //return redirect()->route('subscriptions.index')->with('success', 'Subscription gelöscht.');
        $business = Business::with(['currency', 'locations', 'subscriptions', 'owner'])->find($id);
