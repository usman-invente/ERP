<div class="col-md-4 tw-mb-5 {{ $package->interval }} tw-relative price_card">
    <div
        ...
        <div class="tw-flex tw-flex-col tw-text-center">
            ...
            <h3
                ...
                @php
                    ...
                    $info = is_array($package->more_info_for_package)
                        ? $package->more_info_for_package
                        : json_decode($package->more_info_for_package, true);
                @endphp
                @if ($package->price != 0)
        ...
        <div class="tw-flex tw-flex-col tw-text-white">
            ...
            @if(!empty($info['consent_dsgvo']))
                <div class="tw-flex tw-gap-2 tw-items-center tw-text-[#1f1f1f]">
                    <i class="fa fa-check tw-text-accent"></i>
                    <span>{{ __('superadmin::lang.consent_dsgvo') }}</span>
                </div>
            @endif

            @if(!empty($info['reminder_function']))
                <div class="tw-flex tw-gap-2 tw-items-center tw-text-[#1f1f1f]">
                    <i class="fa fa-check tw-text-accent"></i>
                    <span>{{ __('superadmin::lang.reminder_function') }}</span>
                </div>
            @endif
            @if(!empty($info['birthday']))
                <div class="tw-flex tw-gap-2 tw-items-center tw-text-[#1f1f1f]">
                    <i class="fa fa-check tw-text-accent"></i>
                    <span>{{ __('superadmin::lang.birthday') }}</span>
                </div>
            @endif
            @if(!empty($info['contract_extension']))
                <div class="tw-flex tw-gap-2 tw-items-center tw-text-[#1f1f1f]">
                    <i class="fa fa-check tw-text-accent"></i>
                    <span>{{ __('superadmin::lang.contract_extension') }}</span>
                </div>
            @endif
