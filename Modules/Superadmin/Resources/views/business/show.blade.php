                                ...
                                <tbody>
                                    @foreach ($business->subscriptions as $subscription)
                                        <tr>
                                            ...
                                            <td>
                                                {{-- <a href="{{ route('business.edit_subscription', ['id' => $subscription->business_id, 'sub_id' => $subscription->id]) }}" class="tw-m-0.5 tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline  tw-dw-btn-info">
                                                    @lang('messages.edit')
                                                </a> --}}
                            
                                                <!-- Löschen Button mit Bestätigung -->
                                                <form action="{{ route('business.destroy_subscription', ['id' => $subscription->business_id, 'sub_id' => $subscription->id]) }}" 
                                                    method="POST" 
                                                    style="display:inline-block;" 
                                                    class="delete_business_subscription_confirmation_form">
                                                  @csrf
                                                  @method('DELETE')
                                                  <button type="submit" class="tw-m-0.5 tw-dw-btn tw-dw-btn-xs tw-dw-btn-outline tw-dw-btn-error">
...
@section('javascript')
    <script type="text/javascript">
        ...
        $(document).on('submit', 'form.delete_business_subscription_confirmation_form', function(e) {
    e.preventDefault(); // Formular wird NICHT sofort abgesendet
    let form = this;
    ...
    swal({
        ...
        if (confirmed) {
            form.submit(); // Absenden, wenn bestätigt
        }
    });
});
