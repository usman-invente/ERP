@extends('layouts.app')
@section('title', __( 'productcatalogue::lang.vollmacht_generate_qr' ))
...
<section class="content-header">
    <h1><i class="fa fas fa-qrcode"></i> @lang( 'productcatalogue::lang.vollmacht_generate_qr' )</h1>
</section>
@php
    ...
    <div class="row">
    	...
    	@component('components.widget', ['class' => 'box-solid'])
            ...
            <div class="form-group">
                ...
                {!! Form::text('subtitle', __('productcatalogue::lang.qr_code_customerregister'), ['class' => 'form-control']); !!}
            </div>
            <div class="form-group">
                <div class="checkbox">
                    <label>
                ...
                <div class="checkbox">
                    <label> 
                        {!! Form::checkbox('add_title_and_subtitle', 1, true, ['id' => 'show_title_and_subtitle', 'class' => 'input-icheck']); !!} @lang('productcatalogue::lang.show_title_and_subtitle_on_qrcode')
                    </label>
                </div>
            </div>
...
<script type="text/javascript">
    ...
    $(document).on('click', '#generate_qr', function(e){
        ...
        if ($('#location_id').val()) {
            ...
            {{-- var link = "{{url('repair/dashboard/')}}"; --}}
            {{-- var link = "{{url('crm-new-customer/'" + $('#business_n').val() +"'/'. session('business.id'))}}/" + $('#location_id').val(); --}}
            {{-- var link = "{{url('crm-new-customer/'.session('business.name').'/'. session('business.id'))}}/" + $('#location_id').val(); --}}
            ...
            if($('#show_title_and_subtitle').is(':checked')){
                if ($('#title').val().trim() !== '' ) {
                    opts.title = $('#title').val();
                    opts.titleFont = "bold 18px Arial";
                    opts.titleColor = "#004284";
