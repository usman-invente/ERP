@extends('layouts.app')
@section('title', __( 'productcatalogue::lang.consent_generate_qr' ))
...
<section class="content-header">
    <h1></i> @lang( 'productcatalogue::lang.consent_generate_qr' )</h1>
</section>
@php
...
<script type="text/javascript">
    ...
    $(document).on('click', '#generate_qr', function(e){
        $('#qrcode').html('');
        if ($('#location_id').val()) {
            var link = "{{url('crm-new-customer-consent/'. session('business.id'))}}/" + $('#location_id').val()+"/" + $('#business_n').val();
            var color = '#000000';
            if ($('#color').val().trim() != '') {
                color = $('#color').val();
            }
