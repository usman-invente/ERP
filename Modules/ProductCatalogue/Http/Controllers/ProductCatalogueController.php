...
class ProductCatalogueController extends Controller
{
    ...
    public function consent_dokumentGenerateQr()
    {
        $business_id = request()->session()->get('user.business_id');
        if (! (auth()->user()->can('superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'productcatalogue_module'))) {
            abort(403, 'Unauthorized action.');
        ...
        return view('productcatalogue::catalogue.consent_dokument_generate_qr')
                    ->with(compact('business_locations', 'business'));
    }

    public function consentGenerateQr()
    {
        $business_id = request()->session()->get('user.business_id');
        if (! (auth()->user()->can('superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'productcatalogue_module'))) {
            abort(403, 'Unauthorized action.');
        ...
        return view('productcatalogue::catalogue.consent_generate_qr')
                    ->with(compact('business_locations', 'business'));
    }

    public function newsletterGenerateQr()
    {
        $business_id = request()->session()->get('user.business_id');
        if (! (auth()->user()->can('superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'productcatalogue_module'))) {
            abort(403, 'Unauthorized action.');
        ...
        return view('productcatalogue::catalogue.newsletter_generate_qr')
                    ->with(compact('business_locations', 'business'));
    }
}
