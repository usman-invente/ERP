                    ...
                    @if(auth()->user()->can('crm.access_all_leads') || auth()->user()->can('crm.access_own_leads'))
                        <li @if(request()->segment(2) == 'leads') class="active" @endif>
                            <a href="{{action([\Modules\Crm\Http\Controllers\LeadController::class, 'index']). '?lead_view=list_view'}}">
                                @lang('crm::lang.customer_and_lead_db')
                            </a>
                        </li>
                    @endif
                    <li @if(request()->segment(2) == 'customer-import') class="active" @endif>
                        <a href="{{action([\Modules\Crm\Http\Controllers\LeadController::class, 'getImportCrmContacts'])}}">
                            @lang('crm::lang.crm_customer_import')
                        </a>
                    </li>
                    @if($business->id == 1 || is_object($active_subscription) && in_array($active_subscription->package_id, $business->getAllowedPackages()))
                    <li 
                        @if(request()->segment(2) == 'settings') class="active" @endif>
